...
(async () => {
  ...
  try {
    ...
    async function getApiKey(force = false) {
      ...
      let encryptedKey = localStorage.getItem('GEMINI_API_KEY');
      let key = null;
      if (encryptedKey) {
        try {
          key = await decrypt(encryptedKey);
        } catch (e) {
    ...
    // ===== 共通化：Gemini API 呼び出し（※ここが欠けると動きません） =====
        ...
        ">
          <div style="display:flex;justify-content:space-between;align-items:center;
            ...
            <span class="gemini-answer-status"
                  style="font-size:0.8em;opacity:0.7;">準備中...</span>
          </div>
...
- 固定の型にこだわりすぎず、「このユーザーが今知りたいこと」が伝わるように、多少フォーマットを崩しても構いません。
    ...
    if (!apiKey) {
      answerStatusEl.textContent = 'キャンセル';
      answerEl.textContent = 'APIキーが未設定のため停止しました。';
      return;
    }
    ...
    const results = await (async function fetchSearchResults(formEl, mainEl, maxResults) {
      let results = Array.from(mainEl.querySelectorAll('.result'));
      let currentResults = results.length;
      let pageNo = parseInt(new FormData(formEl).get('pageno') || 1, 10);
    ...
    })(form, mainResults, CONFIG.MAX_RESULTS);
